...
public class SmsAuthenticatorFactory implements AuthenticatorFactory {
	...
	public List<ProviderConfigProperty> getConfigProperties() {
		return List.of(
			...
			new ProviderConfigProperty("maxVerifyAttempts", "Max verification attempts", "Maximum number of wrong OTP entries (counted across all login sessions for the user) before the OTP step is locked. Defaults to 5.", ProviderConfigProperty.STRING_TYPE, "5"),
			new ProviderConfigProperty("otpLockoutSeconds", "OTP lockout duration (seconds)", "How long the OTP step stays locked for a user after the maximum wrong attempts is reached. Also the rolling window for the send budget. Defaults to 900 (15 minutes).", ProviderConfigProperty.STRING_TYPE, "900"),
			new ProviderConfigProperty("maxOtpSends", "Max OTP sends per window", "Maximum number of OTP SMS that may be sent to a user within the lockout window (counted across all login/registration sessions). Defaults to 5.", ProviderConfigProperty.STRING_TYPE, "5"),
